...
'''随机生成区块 按照备份数量生成'''
def generate_blockAllo_backups(best_solution,blockBackupsUpper,nodeNum, blockNum, blockBackups, blockInfo, nodeLimit, storageLimitcondition, storageLimit,alloNum=10):
...
全局变量 costAll
'''
def objective(blockAllo, blockNum, nodeNum, costAll, blockInfo, nodeLimit, node_cost_ratio):
...
'''计算个体的适应度'''
def cal_fitness(executor,blockAlloGen, blockNum,nodeNum, costAll, blockInfo,nodeLimit, node_cost_ratio):
    alloNum = blockAlloGen.shape[0]
    #行代表一种分配/个体 列依次为 适应度 总目标 通信成本 存储平衡度 
    alloTarget = np.zeros((alloNum,3)) 
    future_results = [executor.submit(objective, blockAllo, blockNum, nodeNum, costAll, blockInfo, nodeLimit, node_cost_ratio) for blockAllo in blockAlloGen] 
    alloTarget = np.array([future.result() for future in future_results])
    # for index,blockAllo in enumerate(blockAlloGen): #遍历
    #     alloTarget[index,:] = objective(blockAllo, blockNum, nodeNum, costAll, blockInfo, nodeLimit, node_cost_ratio)
...
def crossover_mutation_individual(alloFa,alloSize, blockAlloGen, blockNum, nodeNum, blockBackups, blockInfo, nodeLimit, storageLimitcondition, storageLimit, CROSSOVER_RATE=0.8, MUTATION_RATE=0.5):
    ...
    if(crossover_flag|mutation_flag):
        constraint_result = constraint(alloChild, blockInfo,nodeLimit, blockBackups,storageLimitcondition, storageLimit )
        #大多数是因为第1个条件不满足要求
        if(constraint_result[1]==False):
            alloChild = fix_constraint1(alloChild,blockBackups)
            constraint_result = constraint(alloChild, blockInfo,nodeLimit, blockBackups,storageLimitcondition, storageLimit )
            
        #孩子2    
        if(crossover_flag):    
            constraint_result = constraint(alloChild2, blockInfo,nodeLimit, blockBackups,storageLimitcondition, storageLimit )
            if(constraint_result[1]==False):
                alloChild = fix_constraint1(alloChild2,blockBackups)
                constraint_result = constraint(alloChild2, blockInfo,nodeLimit, blockBackups,storageLimitcondition, storageLimit )
            
        return alloChild, alloChild2 if crossover_flag else None
    return None, None

'''交叉变异'''
def crossover_mutation(executor,blockAlloGen,blockNum,nodeNum,blockBackups,blockInfo,nodeLimit,storageLimitcondition,storageLimit, CROSSOVER_RATE=0.8, MUTATION_RATE=0.5):
    blockAlloNew = []
    alloSize = blockAlloGen.shape[0]

    future_results = [executor.submit(crossover_mutation_individual, alloFa,alloSize, blockAlloGen, blockNum, nodeNum, blockBackups, blockInfo, nodeLimit, storageLimitcondition, storageLimit, CROSSOVER_RATE, MUTATION_RATE) for alloFa in blockAlloGen]
    for alloFa in blockAlloGen:
        blockAlloNew.append(alloFa)
    for future in future_results:
        alloChild, alloChild2 = future.result()
        if alloChild is not None:
            blockAlloNew.append(alloChild)
        if alloChild2 is not None:
...
'''
def fix_constraint1(blockAllo, blockBackups):
...
def evaluate(start_time,nodeNum, blockNum, storageLimit, alloNum, epochNum,blockBackups, node_cost_ratio = 1, best_solution = None):
    #区块信息
    ...
    blockAlloGen = generate_blockAllo_backups(best_solution,blockBackups+1,nodeNum, blockNum, blockBackups, blockInfo, nodeLimit,storageLimitcondition, storageLimit, alloNum) #生成原始种群
    ...
    #迭代
        for epoch in range(epochNum):
            alloFit,alloTarget = cal_fitness(executor,blockAlloGen, blockNum,nodeNum, costAll, blockInfo, nodeLimit, node_cost_ratio) #计算种群的 适应度 总目标 通信成本 存储平衡度
            alloMinIndex = np.argmin(alloTarget[:,0])
            alloEpoch[epoch,:] = np.concatenate((alloTarget[alloMinIndex,:], [time.time() - start_time]))  # 总目标 通信成本 存储平衡度
            ...
            #交叉变异 生成下一轮的个体
            blockAlloGen = crossover_mutation(executor,blockAlloGen,blockNum=blockNum,nodeNum=nodeNum,blockBackups=blockBackups,blockInfo=blockInfo,nodeLimit=nodeLimit,storageLimitcondition=storageLimitcondition,storageLimit=storageLimit, CROSSOVER_RATE=0.8)
    ...
    #最后一轮
        alloFit,alloTarget = cal_fitness(executor,blockAlloGen,blockNum,nodeNum, costAll, blockInfo, nodeLimit, node_cost_ratio) #计算种群的 适应度 总目标 通信成本 存储平衡度
        alloMinIndex = np.argmin(alloTarget[:,0])
        alloEpoch[epochNum,:] = np.concatenate((alloTarget[alloMinIndex,:], [time.time() - start_time]))  # 总目标 通信成本 存储平衡度
        bestBlockAllo = blockAlloGen[alloMinIndex,:,:] #这一轮的最优分配     
        ...
        return alloEpoch,bestBlockAllo
...
def getResultWorker1(blockNum,nodeNum,blockBackups,storageLimit,alloNum,epochNum):
    fileSaveName = 'Genetic_B{0}_N{1}_BU{2}_L{3}_AN{4}_E{5}_workers_1'.format(blockNum,nodeNum,blockBackups,storageLimit,alloNum,epochNum)
    result = np.load('./IterationChart/Q2/'+fileSaveName+'-alloEpoch.npz')
    return result['alloEpoch']

def getResultWorker10(blockNum,nodeNum,blockBackups,storageLimit,alloNum,epochNum):
    fileSaveName = 'Genetic_B{0}_N{1}_BU{2}_L{3}_AN{4}_E{5}_workers_10'.format(blockNum,nodeNum,blockBackups,storageLimit,alloNum,epochNum)
    result = np.load('./IterationChart/Q2/'+fileSaveName+'-alloEpoch.npz')
    return result['alloEpoch']
